    ...
    #define DEVICE_NAME                     "SomeDevice"                                /**< Name of device. Will be included in the advertising data. */
            ...
            case BLE_GAP_EVT_CONN_PARAM_UPDATE:
             ...
             break;
              ...
              case BLE_EVT_TX_COMPLETE:
                if(buffer_full)
                {
                    err_code = ble_nus_string_send(&m_nus, data_array, buf_index);
                ...
                while(app_uart_get(&data_array[buf_index]) != NRF_ERROR_NOT_FOUND)
                {
                    buf_index++;
                    if ((data_array[buf_index] == '\n') || (buf_index >= (BLE_NUS_MAX_DATA_LEN)))
                    {
                        err_code = ble_nus_string_send(&m_nus, data_array, buf_index);
                        if(err_code == BLE_ERROR_NO_TX_BUFFERS)
                        {
    ...
    void uart_event_handle(app_uart_evt_t * p_event)
    {
        ...
        switch (p_event->evt_type)
        {
            case APP_UART_DATA_READY:
                if(!buffer_full)
                {
                    UNUSED_VARIABLE(app_uart_get(&data_array[buf_index]));
                    buf_index++;

                    if ((data_array[buf_index - 1] == '\n') || (buf_index >= (BLE_NUS_MAX_DATA_LEN)))
                    {
                        err_code = ble_nus_string_send(&m_nus, data_array, buf_index);
                        if(err_code == BLE_ERROR_NO_TX_BUFFERS)
                        {
    ...
    int main(void)
    {
        ...
        nrf_gpio_cfg_output(GPIO_ANT_PIN_NUMBER);
        nrf_gpio_pin_clear(GPIO_ANT_PIN_NUMBER);
